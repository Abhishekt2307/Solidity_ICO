...
contract ICO is Ownable {
    ...
    function pause_unpause() public onlyOwner {
        pause = !pause;
    }
    ...
    function vestingPeriodTokens() private view  returns(uint256,uint256)
    {
        ...
        unchecked
        {
        uint256 currentParts=(block.timestamp-myPhases[_maxPhase].closingTime)/30;
        if(currentParts>10)currentParts=10;
        uint256 part=(currentParts)-userTokens[msg.sender].partsTaken;
    ...
    function claimTokens() public 
    {
        ...
        require(block.timestamp>myPhases[_maxPhase].closingTime,"wait for ICO to end to claim yr tokens");
        require((block.timestamp-myPhases[_maxPhase].closingTime)>=30,"please wait for minimum 30 sec");
        require(userTokens[msg.sender].tokensLeft!=0,"you have already claimed all yr tokens");
